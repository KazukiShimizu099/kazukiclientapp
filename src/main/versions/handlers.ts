...
// Map new year-based IDs to their internal Mojang IDs if different
// Mojang manifest uses internal IDs like "1.21.4" even for new naming scheme
...
async function downloadLibraries(libraries: any[], gameDir: string, win: BrowserWindow | null) {
  ...
  for (let i = 0; i < validLibs.length; i++) {
    ...
    win?.webContents.send('download:progress', {
      name: lib.name, downloaded: i+1, total: validLibs.length,
...
async function downloadAssets(assetIndex: any, assetIndexId: string, gameDir: string, win: BrowserWindow | null) {
  ...
  async function worker() {
    while (queue.length > 0) {
      ...
      try { await downloadFile(`${RESOURCES_URL}/${prefix}/${obj.hash}`, dest) } catch {}
      done++
      if (done % 20 === 0) {
        win?.webContents.send('download:progress', {
          name:'Assets', downloaded:done, total, percent:Math.round((done/total)*100)
        })
      }
    }
  }
...
export function setupVersionHandlers(ipcMain: IpcMain, store: any) {
  ipcMain.handle('versions:get-list', async () => {
    try {
      ...
      // Include all release versions from 1.12.2 and newer year-based versions
      const supported = manifest.versions.filter((v: any) => {
        if (v.type !== 'release') return false
        const id = v.id
        // Year-based (26.x, 27.x, etc.)
        if (/^\d{2,}\./.test(id)) return true
        // Old 1.x.x format - only 1.12.2 and above
  ...
  ipcMain.handle('versions:install', async (event, versionId: string) => {
    ...
    try {
      ...
      // Try to find by id directly, or by mapped id
      ...
      const versionRes = await axios.get(versionMeta.url, { timeout: 15000 })
      const versionData = versionRes.data

      // Use the actual ID from manifest (not our display name)
      ...
      // Save mapping if display ID differs from actual ID
